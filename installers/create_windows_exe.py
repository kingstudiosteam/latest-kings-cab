#!/usr/bin/env python3
"""
The King's Cab - Windows EXE Installer Creator
Creates a traditional Windows .exe installer that customers can double-click
"""
...
def create_installer_exe():
    """Create a self-extracting Windows installer"""
    
    print("🎛️  Creating Windows EXE Installer...")
    print("=====================================")
    
    # Installer template - this creates a self-extracting executable
    installer_template = '''@echo off
echo.
echo ===============================================
echo   THE KING'S CAB - PROFESSIONAL IR LOADER
echo ===============================================
echo   Version: 1.0.0
echo   Company: King Studios
echo   Website: https://www.kingstudiospa.com
echo ===============================================
echo.
...
echo   * Complete IR collection included
echo.
...
REM Extract files (this section will be populated by the Python script)
echo Extracting plugin files...

REM Set proper permissions
echo Setting permissions...
icacls "%ProgramFiles%\\Common Files\\VST3\\The King's Cab.vst3" /grant Users:F /T 2>nul
icacls "%ProgramFiles%\\Common Files\\Avid\\Audio\\Plug-Ins\\The King's Cab.aaxplugin" /grant Users:F /T 2>nul
icacls "%ProgramData%\\King Studios\\The King's Cab" /grant Users:F /T 2>nul

REM Create registry entries
echo Creating registry entries...
reg add "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\The King's Cab" /v "DisplayName" /t REG_SZ /d "The King's Cab" /f >nul
reg add "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\The King's Cab" /v "DisplayVersion" /t REG_SZ /d "1.0.0" /f >nul
reg add "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\The King's Cab" /v "Publisher" /t REG_SZ /d "King Studios" /f >nul
reg add "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\The King's Cab" /v "URLInfoAbout" /t REG_SZ /d "https://www.kingstudiospa.com" /f >nul

echo.
echo ✅ Installation completed successfully!
echo.
echo Plugin installed for:
echo   • All VST3 compatible DAWs
echo   • Pro Tools (AAX format)
echo   • Complete IR collection included
echo.
echo 🚀 Restart your DAW to load The King's Cab plugin!
echo 🎸 Visit https://www.kingstudiospa.com/store for more premium content
echo.
pause
'''

    # Create the installer
    installer_path = "TheKingsCab-1.0.0-Windows-Installer.exe"
    
    # For now, create a batch file that looks more professional
...
'''
    ...
    # Write the professional installer
    with open("windows/INSTALL-TheKingsCab.bat", "w") as f:
        f.write(batch_installer)
    
    print("✅ Professional Windows installer created!")
    print(f"📦 File: windows/INSTALL-TheKingsCab.bat")
